function [data_sol, path_sol] = TERRA_3D(showResults, home, xy, last_depot, uav_data, ugv_data, saveResults, fullname)
...
%java -jar -Djava.util.Arrays.useLegacyMergeSort=true -Xmx1G D:\OneDrive\Universidad\Doctorado\Matlab\Path-Planning\PathPlanning.jar D:\OneDrive\Universidad\Doctorado\Matlab\3DMaps\DTEED_030808_1535_031230_1535_A01.IMG null 1000 1000 1020 1020 -n 0 -z ugv_path
...
if (showResults)
    close all
    fig1 = figure('Name','TERRA | Results','Numbertitle','off','Position', [1600, 40, 900, 750]);
    subplot(2,2,1);
...
% - Compute 3Dana for each vertice of the TSP solution
...
[route, ~, xy] = tsp_ga_ugv(x, y, path_x, path_y, last_depot, showResults, 'Without GO', saveResults, fullname);

ugv_path = [xy(route,1) xy(route,2)]; %UGV Path

%Compute 3Dana and extract the whole ugv path
[ugv_path, ugv_distance] = compute_3Dana(ugv_path);

ugv_time = ugv_distance/ugv_data.Vugv; %UGV Time

%UAV Path
[uav_path1, uav_path2, uav_distance, uav_time, stops] = uav_compute_path3D(xy, route, home_x, home_y, x, y, wp_c, scp_table, SolL, [vx_sol;vy_sol], showResults, ugv_path, uav_data, 'Without GO', saveResults, fullname);
