%  | MODIFIED VERSION |
%  function [solution_setsLabelsV, scp_table, V2] = greedy_scp_3D(T, V, R)
%  It computes the set covering problem as the original explained
%  below, but with different INPUTS & OUTPUTS.
%  Inputs:
%    T(x;y;z)    - set of target points in meters
%    V           - set of vertices solved in Voronoi Tessellations
%    R           - Radius defining the maximum distance the UAV can travel in meters
%    cfg_params    - testing configuration parameters
%  Outputs:
%    solution_setsLabelsV  - If a L vector of integer labels of the elements of C is provided, 
%                            SolL contains the labels corresponding to SolC. 
%    scp_table             - Solution table with every T covered by a
%                            vertice in V
%    V2                    - set of vertices solved in this step in meters
%    figG                  - solution figure
function [solution_setsLabelsV, scp_table, V2, figG] = greedy_scp_3D(T, V, R, cfg_params)
% | ORIGINAL VERSION |
...
for i=1:ty
    for j=1:vy
        d = sqrt( ((T(1,i)-V(1,j))^2) + ((T(2,i)-V(2,j))^2) + ((T(3,i)-V(3,j))^2));
        if (int64(d) <= R)
...
if (c>0)
    ...
    for i=1:c
        c_x(i,:) = R*cos(theta) + V2(1,i);
        c_y(i,:) = R*sin(theta) + V2(2,i);
        plot(c_x(i,:),c_y(i,:),'r:');
    end
end
hold off
