% Same as foptimus_3D, but in 2-D. 

% INPUTS
% a        - vector with the a's center of the WPs circunferences C(a,b)
% b        - vector with the b's center of the WPs circunferences C(a,b)
...
% * This function is applied in 2D Maps, where there is not altitudes map.

function [Vopt] = foptimus_2D(a, b, Xm, Ym, R, map_data)
 ...
 for j=1:r
     wp_in = 0;
     for k=1:length(a)
         d = sqrt( ((jp_sol(j,1)-a(k))^2) + ((jp_sol(j,2)-b(k))^2) );
         if (round(d) <= R)
             wp_in = wp_in + 1;
         end
     end
     if (wp_in == length(a))
         jp = struct('id',j,'x',jp_sol(j,1),'y',jp_sol(j,2),'z',jp_sol(j,3),'d',sqrt(((jp_sol(j,1)-Xm(2))^2) + ((jp_sol(j,2)-Ym(2))^2)));
...
while (~isempty(jp_cover))
    ...
    if (inGreenZone([current_jp.x;current_jp.y],map_data))
        ...
        %Line between the junction point [current_jp] and the vertice [Xm(1);Ym(1)]
...
function v_opt = axiSearch(eqL,eqM,var,Gp,jp_coordinate,v_coordinate,map_data,jp,vertex,id)
    ...
    %Distance vertex - gp
    d1 = sqrt( ((Gp(1,1)-vertex(1,1))^2) + ((Gp(1,2)-vertex(1,2))^2));
    %Distance jp - gp
    d2 = sqrt( ((jp.x-vertex(1,1))^2) + ((jp.y-vertex(1,2))^2));
        ...
        if (i~=jp_coordinate)
            ...
            if (var=='x')
                ...
                if (inGreenZone([i;value],map_data))
                    v_opt = struct('id',id,'x',double(i),'y',value,'z',double(map_data.z_map(i,value)),'d',sqrt( ((double(i)-Gp(1,1))^2) + ((value-Gp(1,2))^2) ));
                    break;
                end
            elseif (var=='y')
                if (eqM == -Inf || eqM == +Inf)
                ...
                if (inGreenZone([value;i],map_data))
                    v_opt = struct('id',id,'x',value,'y',double(i),'z',double(map_data.z_map(value,i)),'d',sqrt( ((value-Gp(1,1))^2) + ((double(i)-Gp(1,2))^2)));
                    break;
                end
            end
        end
